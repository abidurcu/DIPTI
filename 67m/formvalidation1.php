<?php 
	$showMessage = $fname = $errFname = $crrfname = $lname = $errLname = $crrlname = $crrname = $email = $errEmail = $crrEmail = $gender = $errGender = $test = $phone = $errphone = $crrphone = $country = $errCountry = $crrcountry = $quata = $crrQuata = $y = null;
	...
	if(!empty($crrname) && !empty($crrEmail) && !empty($gender) && !empty($phone) && !empty($crrcountry)){
		$showMessage = $crrname.$crrEmail.$gender.$crrphone.$crrcountry.$crrQuata;
		$y = 1;
	}
}
 ?>
